...
export const Carousel = ({ thumbnail, images, duration }: ImageCarouselProps) => {
  const imageSlider: string[] = [thumbnail, ...images];

  const { activeImage, activeSlide, changeActiveSlideImageWhenClick } = useSlider(imageSlider, duration);
        ...
        {imageSlider.map((_, i) => {
          return (
            <li
              key={i}
              className={`${
                activeSlide === i ? 'bg-slate-900' : 'bg-slate-700'
...
Carousel.defaultProps = { duration: 3 };
